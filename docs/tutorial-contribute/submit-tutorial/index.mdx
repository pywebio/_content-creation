...
*Jeff Terrell, April 7, 2022*


Submitting a tutorial doesn't have to be difficult, and hopefully you'll find the process easy to get through. This tutorial will guide you through the tutorial creation process step by step resulting in a completed and fully deployed tutorial on PyWebIO. Let's dive in!
...
Let's begin with forking the pywebio/content-creation repository which is where your tutorial will eventually be pushed to and deployed from.
...
Navigate to the docs directory of the repo and find the category directory that your tutorial matches. From this directory click the "Add File" button, and then select "Create new file". If there is no matching category, you will need to create one which requires a few extra steps (click here for these steps). If there is a matching category continue reading.
...
#### Creating Tutorial Category (if needed)
First, make sure you are in the docs directory, click the "Add File" button, and then select "Create new file". The category directory should be created with this path (content_creation/docs/newcategory) and the tutorial directory be named by the subject matter of the tutorial (e.g., setup-web3). The tutorial file should be named index.mdx. Example:
* ```content_creation/docs/newcategory/setup-web3/index.mdx```

The final step is to create a Json file which is used to give the category a title and order position in the left navbar. The Json file belongs in the category directory and must be created exactly as shown below:
* ```_category_.json```

And now we need to add a little code to the file and we're done!
```
{
  "label": "New Category",
  "position": 5
...
Scroll to the bottom of the page and in the Commit new file box, select the "Create a new branch for this commit and start a pull request" option. This will give you the option to name the new branch you are creating. The branch should be named with the following format (tutorial-date-description). Example:
* ```tutorial-2022-04-07-description```
