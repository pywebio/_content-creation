...
slug: submit-blog-post # slug is the file path to the tutorial after /docs, should be unique to the document
title: Send Me The Blog Post Testing # main heading that shows at the top of the tutorial
authors: [terrell]
---
...
* content_creation/blog/2022-04-07/index.md
...
Your index.md file should look similar to the screenshot above after copying and pasting the template Markdown code.

You can now begin creating your own blog post! Feel free to improvise and make it your own, the template is not the "law", be creative and have fun but try to follow the general layout of the template as well (thanks :smile:). A few things to note:
* The code in between the three dashes is the front matter field
* slug: 

Note, there is a preview tab next to the Edit file tab that allows you to see how your blog post will appear after being deployed.



## Adding Images And Other Assets
Of course you will likely want to add images or other assets to your blog post. All of the assets need to be saved in the directory containing the index.md file. 

![GitHub Upload Files](./gh_upload_files.JPG)

Make sure you are on the branch specific to your blog post and in the correct directory (blog/date-description), click the "Add file" button and select "Upload files". You will have the option to either drog and drop or choose the files from a file explorer. You can upload as many files as needed from this screen. After you are finished, ensure the ***"Needs edit: Commit directly to the 'your branch' branch"*** option is selected and click the "Commit changes" button.

You will be directed to the home screen of the branch you just committed to, simply navigate back to the index.md file in the blog directory to continue editing your blog post. Also, because the uploaded assets reside in the same directory as your index.md file they are very easy to access. Simply use the following relative file path:
* ./your_asset.jpg
...
```
Add an image by using the exclamation mark(!), followed by alt text in brackets, and the relative or URL to the image in parentheses(). A title can be added in quotation marks("") as well after the relative path or URL
...
Wrap your code in three backticks ``` to use a code snippet. Here's the source code from the code snippet show above.
